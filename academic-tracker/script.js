...
deleteSubjectBtn.addEventListener('click', async () => {
    if (currentSubjectId) {
        try {
            ...
            if (subject) {
                // Get grade count
                const grades = await loadGrades(currentSubjectId);
                const itemCount = grades.length;
                
                const message = `Are you sure you want to delete "${subject.name}"?${itemCount > 0 ? `\n\nThis will also delete all ${itemCount} ${itemCount === 1 ? 'grade item' : 'grade items'} associated with this subject.` : ''}\n\nThis action cannot be undone.`;
